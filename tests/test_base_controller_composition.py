...
def test_basic_composition_lifecycle() -> None:
    ...
    comp.register_controllers(controller1, controller2)
    ...
    assert not controller1._is_disposed
    assert not controller2._is_disposed
    ...
    assert controller1._is_disposed
    assert controller2._is_disposed
...
def test_idempotent_disposal() -> None:
    ...
    comp.register_controllers(controller)
    
    # First disposal
    comp.dispose()
    assert comp.is_disposed
    assert controller._is_disposed
...
def test_auto_register_controllers() -> None:
    ...
    assert source._private_controller not in comp.controllers
...
def test_nested_composition_basic() -> None:
    ...
    child_comp_a.register_controllers(child_controller_a1, child_controller_a2)
    child_comp_b.register_controllers(child_controller_b1, child_controller_b2)
    ...
    assert not any(c._is_disposed for c in [child_controller_a1, child_controller_a2, child_controller_b1, child_controller_b2])
    assert not standalone_controller._is_disposed
    ...
    assert standalone_controller._is_disposed
    assert child_controller_a1._is_disposed
    assert child_controller_a2._is_disposed
    assert child_controller_b1._is_disposed
    assert child_controller_b2._is_disposed
...
def test_nested_composition_auto_registration() -> None:
    ...
    child_comp.register_controllers(child_controller)
...
def test_nested_composition_disposal_order() -> None:
    ...
    class TrackingController(DisplayValueController):
    ...
    class TrackingComposition(BaseControllerComposition):
        def __init__(self, name: str) -> None:
            super().__init__()
            self.name = name
...
def test_disposal_with_errors() -> None:
    ...
    class ErrorController(DisplayValueController):
        def dispose(self) -> None:
    ...
    comp.register_controllers(good_controller, bad_controller)
    ...
    assert comp.is_disposed
    assert good_controller._is_disposed
    assert bad_controller._is_disposed
...
def test_context_manager() -> None:
    ...
    with BaseControllerComposition() as comp:
        comp.register_controllers(controller)
        assert not comp.is_disposed
        assert not controller._is_disposed
...
def test_deeply_nested_composition() -> None:
    ...
    level3.register_controllers(level3_controller)
    
    # Add level 3 to level 2
    level2_controller = CheckBoxController(True, text="level2")
    level2.register_controllers(level2_controller, level3)
    
    # Add level 2 to level 1
    level1_controller = TextEntryController("level1")
    level1.register_controllers(level1_controller, level2)
    
    # Check initial state
    assert not any(comp.is_disposed for comp in [level1, level2, level3])
    assert not any(ctrl._is_disposed for ctrl in [level1_controller, level2_controller, level3_controller])
    ...
    assert all(ctrl._is_disposed for ctrl in [level1_controller, level2_controller, level3_controller])
...
def test_type_restrictions() -> None:
    ...
    for invalid_obj in invalid_objects:
        with pytest.raises(TypeError, match="Only BaseController and BaseControllerComposition instances can be registered"):
            comp.register_controllers(invalid_obj)
    ...
    comp.register_controllers(valid_controller, valid_composition)
    assert len(comp.controllers) == 2
...
def test_disposal_order_independence() -> None:
    ...
    comp1.register_controllers(controller1, controller2, child_comp1, child_comp2)
    
    # Track disposal order
    disposal_order = []
    
    def track_disposal(controller, name):
        original_dispose = controller.dispose
        def tracked_dispose():
            disposal_order.append(name)
            original_dispose()
        controller.dispose = tracked_dispose
    ...
    def tracked_dispose1():
        disposal_order.append("ChildComp1")
        original_dispose1()
    
    def tracked_dispose2():
        disposal_order.append("ChildComp2")
        original_dispose2()
    ...
    comp2.register_controllers(child_comp3, child_comp4, controller3, controller4)
    
    disposal_order2 = []
    
    def track_disposal2(controller, name):
        original_dispose = controller.dispose
        def tracked_dispose():
            disposal_order2.append(name)
            original_dispose()
        controller.dispose = tracked_dispose
    ...
    def tracked_dispose3():
        disposal_order2.append("ChildComp3")
        original_dispose3()
    
    def tracked_dispose4():
        disposal_order2.append("ChildComp4")
        original_dispose4()
