"""Tests for DictSelectionController class."""
...
def test_dict_selection_controller_initialization_with_direct_value(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    """Test that DictSelectionController initializes correctly with direct dictionary value."""
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_initialization_first_key_default(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    """Test that DictSelectionController initializes with first key when no selection provided."""
    controller = DictSelectionController(
        sample_dict,
        debounce_ms=TEST_DEBOUNCE_MS
    )
    
    # Check that some key is selected (exact key depends on dict ordering)
    assert controller.selected_key is not None
    assert controller.selected_key in sample_dict
    assert controller.selected_value == sample_dict[controller.selected_key]


@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_initialization_with_observable(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    """Test that DictSelectionController initializes correctly with observable."""
    observable = ObservableSelectionDict[str, str](sample_dict, "banana")
    controller = DictSelectionController(
        observable,
        debounce_ms=TEST_DEBOUNCE_MS
    )
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_initialization_with_hook(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    """Test that DictSelectionController initializes correctly with hook."""
    observable = ObservableSingleValue[str]("orange")
    controller = DictSelectionController(
        sample_dict,
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_initialization_empty_dict_raises_error(qtbot: QtBot) -> None:
    """Test that DictSelectionController raises error with empty dictionary."""
    with pytest.raises(ValueError, match="Dictionary cannot be empty"):
        DictSelectionController(
            {},
            debounce_ms=TEST_DEBOUNCE_MS
        )
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_selected_key_setter(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_change_selected_key(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_dict_like_interface(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_dict_modification(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_delete_key(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_delete_last_key_raises_error(qtbot: QtBot) -> None:
    """Test that deleting the last key raises error."""
    controller = DictSelectionController(
        {"apple": "red"},
        selected_key="apple",
        debounce_ms=TEST_DEBOUNCE_MS
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_change_dict_value(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_change_dict_and_key(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_change_all(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_formatter(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    ...
    controller.formatter = lambda key: f"Item: {key}" # type: ignore
    assert controller.formatter("banana") == "Item: banana" # type: ignore


@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_hooks(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_copy(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_integer_keys(qtbot: QtBot, sample_dict_int: dict[int, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_error_handling(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_add_values_callback(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
...
@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_widget_creation(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    ...
    # Test that combobox has correct number of items (no None option)
    assert controller.widget_combobox.count() == len(sample_dict)


@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_pop_only_key_raises_error(qtbot: QtBot) -> None:
    """Test that popping the only key raises error."""
    controller = DictSelectionController(
        {"apple": "red"},
        selected_key="apple",
        debounce_ms=TEST_DEBOUNCE_MS
    ...
    # Cannot pop the only key
    with pytest.raises(ValueError, match="Cannot pop key 'apple' because it is currently selected"):
        controller.pop("apple")


@pytest.mark.qt_log_ignore(".*")
def test_dict_selection_controller_change_dict_to_empty_raises_error(qtbot: QtBot, sample_dict: dict[str, str]) -> None:
    """Test that changing dict to empty raises error."""
    controller = DictSelectionController(
        sample_dict,
        selected_key="apple",
        debounce_ms=0  # No debounce for immediate error
    )
    
    # Cannot set empty dict - should raise KeyError when trying to complete the submission
    with pytest.raises(KeyError, match="Selected key apple not in new dictionary"):
        controller.submit_values({"dict": {}}, raise_submission_error_flag=False)
