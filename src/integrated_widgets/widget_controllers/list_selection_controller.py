...
# BAB imports
from observables import ObservableSingleValueProtocol, ObservableSetProtocol, ObservableSelectionOptionProtocol, HookProtocol
...
class ListSelectionController(BaseComplexHookController[Literal["selected_option", "available_options"], Any, Any, Any, "ListSelectionController"], ObservableSelectionOptionProtocol[T], Generic[T]):
    """
    ...
    selected_option : T | HookProtocol[T] | ObservableSingleValueProtocol[T] | ObservableSelectionOptionProtocol[T]
        ...
        - A HookProtocol object for bidirectional synchronization
        - An ObservableSingleValueProtocol for synchronization
        - An ObservableSelectionOptionProtocol that provides both selected and available options
    available_options : set[T] | HookProtocol[set[T]] | ObservableSetProtocol[T] | None
        ...
        - None only if selected_option is ObservableSelectionOptionProtocol
    ...
    ------
    ValueError
        If available_options is provided when selected_option is ObservableSelectionOptionProtocol.
    ValueError
    ...
    selected_option_hook : OwnedHookProtocol[T]
        Hook for the selected option that can be connected to observables.
    available_options_hook : OwnedHookProtocol[set[T]]
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(selected_option, ObservableSelectionOptionProtocol):
            log_msg(self, "__init__", logger, "selected_option is ObservableSelectionOptionProtocol")
            ...
            log_msg(self, "__init__", logger, f"From ObservableSelectionOptionProtocol: initial_selected_option={initial_selected_option}, initial_available_options={initial_available_options}")

        else:
            log_msg(self, "__init__", logger, "selected_option is not ObservableSelectionOptionProtocol, processing manually")
            if isinstance(selected_option, HookProtocol):
                # It's a hook - get initial value
                log_msg(self, "__init__", logger, "selected_option is HookProtocol")
                initial_selected_option: T = selected_option.value # type: ignore
                hook_selected_option: Optional[HookProtocol[T]] = selected_option # type: ignore
                log_msg(self, "__init__", logger, f"From HookProtocol: initial_selected_option={initial_selected_option}")
                ...
                initial_selected_option: T = selected_option.value # type: ignore
                hook_selected_option: Optional[HookProtocol[T]] = selected_option.hook # type: ignore
            ...
            if isinstance(available_options, set):
                ...
                # It's a hook - get initial value
                log_msg(self, "__init__", logger, "available_options is HookProtocol")
                initial_available_options = available_options.value # type: ignore
                hook_available_options = available_options
                log_msg(self, "__init__", logger, f"From HookProtocol: initial_available_options={initial_available_options}")
    ...
    def selected_option_hook(self) -> HookWithOwnerProtocol[T]:
        ...
        hook: HookWithOwnerProtocol[T] = self.get_hook("selected_option") # type: ignore
        log_msg(self, "selected_option_hook.getter", self._logger, f"Getting selected_option_hook: {hook}")
        return hook
    
    @property
    def available_options_hook(self) -> HookWithOwnerProtocol[set[T]]:
        """Get the hook for the available options."""
        hook: HookWithOwnerProtocol[set[T]] = self.get_hook("available_options") # type: ignore
